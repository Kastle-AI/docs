...
```json
{
  ...
  "borrowerPhone": "+15552359854",
  "propertyAddress": "855 Brannan Updated",
  "propertyCity": "San Francisco",
  "propertyState": "CA",
  "propertyZipcode": "94103",
  "customFields": {
...
## Relcu Integration Endpoints

The Relcu Integration API provides endpoints for managing Relcu integrations and handling lead data from Relcu systems. These endpoints allow you to create, manage, and interact with Relcu integrations within Kastle campaigns.

### Authentication

All Relcu integration endpoints require JWT authentication. Include your JWT token in the Authorization header:
...
- **Integration Management Endpoints**: Require `kastle-admin` organization access
- **Lead Management Endpoints**: Require access to Relcu-integrated organizations (`relcu`, `cardinal-financial`)

---

## Integration Management

### Create Relcu Integration

Create a new Relcu integration for a journey configuration.

#### Endpoint
```
POST /relcu/integrations
```

#### Authorization
Requires `kastle-admin` organization access.

#### Request Body
```json
{
  "journey_config_id": "journey_123",
  "relcu_org_id": "relcu_org_456",
  "fetch_leads_url": "https://api.relcu.com/leads",
  "fetch_leads_auth_token": "auth_token_789"
}
```

#### Required Fields
- `journey_config_id` (string) - Journey configuration identifier
- `relcu_org_id` (string) - Relcu organization identifier
- `fetch_leads_url` (string) - URL for fetching leads from Relcu
- `fetch_leads_auth_token` (string) - Authentication token for Relcu API
...
- `401 Unauthorized` - Invalid JWT or insufficient permissions
- `403 Forbidden` - Journey config does not belong to user
- `404 Not Found` - Journey config not found
- `409 Conflict` - Integration already exists for this journey config
- `500 Internal Server Error` - Failed to create integration

---

### Get Relcu Integration

Retrieve a specific Relcu integration by journey configuration ID.

#### Endpoint
```
GET /relcu/integrations/{journey_config_id}
```
...
#### Path Parameters
- `journey_config_id` (string, required) - Journey configuration identifier
...
- `401 Unauthorized` - Invalid JWT or insufficient permissions
- `403 Forbidden` - Journey config does not belong to user
- `404 Not Found` - Journey config or integration not found
- `500 Internal Server Error` - Internal server error

---

### Update Relcu Integration

Update an existing Relcu integration.

#### Endpoint
```
PUT /relcu/integrations/{journey_config_id}
```
...
```json
{
  "relcu_org_id": "relcu_org_789",
  "fetch_leads_url": "https://api.relcu.com/v2/leads",
  "fetch_leads_auth_token": "new_auth_token_123"
}
```
...
- `403 Forbidden` - Journey config does not belong to user
- `404 Not Found` - Journey config or integration not found
- `500 Internal Server Error` - Failed to update integration

---

### Delete Relcu Integration

Delete a Relcu integration.

#### Endpoint
```
DELETE /relcu/integrations/{journey_config_id}
```
...
#### Authorization
Requires access to Relcu-integrated organizations (`relcu`, `cardinal-financial`).
...
7. **Organization Restrictions**: Different endpoints require different organization access levels:
   - Integration management: `kastle-admin`
   - Lead management: `relcu`, `cardinal-financial`
   - Callbacks: `relcu`
